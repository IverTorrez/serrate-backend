...
class ListadoCausasOrdenPronuncioAbogadoService
{
    //Servicio de listado de causas que tienen ordenes pronuncio abogado
    public function listarCausasConOrdenesPronuncioAbogadoUsuario(Request $request)
    {
    ...
    public function listarCausasConOrdenesPronuncioAbogadoAbogadoDependiente(Request $request)
    {
        $usuarioId = Auth::user()->id;
        $query = Causa::where('es_eliminado', 0)
    ...
    public function listarCausasConOrdenesPronuncioAbogadoProcurador(Request $request)
    {
        $query = Causa::where('es_eliminado', 0)
    ...
    public function listarCausasConOrdenesPronuncioAbogadoGeneral(Request $request)
    {
        $query = Causa::where('es_eliminado', 0)
            ->whereHas('ordenes', function ($query) {
                $query->where('etapa_orden', EtapaOrden::PRONUNCIO_ABOGADO)
    ...
    public function devuelveListadoCausasOrdenPronuncioAbogado(Request $request)
    {
        $tipoUsuario = Auth::user()->tipo;
        if ($tipoUsuario === TipoUsuario::ABOGADO_INDEPENDIENTE || $tipoUsuario === TipoUsuario::ABOGADO_LIDER); {
            $causas = $this->listarCausasConOrdenesPronuncioAbogadoUsuario($request);
        }
        if ($tipoUsuario === TipoUsuario::ABOGADO_DEPENDIENTE) {
            $causas = $this->listarCausasConOrdenesPronuncioAbogadoAbogadoDependiente($request);
        }
        if ($tipoUsuario === TipoUsuario::PROCURADOR) {
            $causas = $this->listarCausasConOrdenesPronuncioAbogadoProcurador($request);
        }
        if ($tipoUsuario === TipoUsuario::ADMINISTRADOR || $tipoUsuario === TipoUsuario::CONTADOR || $tipoUsuario === TipoUsuario::OBSERVADOR || $tipoUsuario === TipoUsuario::PROCURADOR_MAESTRO) {
            $causas = $this->listarCausasConOrdenesPronuncioAbogadoGeneral($request);
        }
        return $causas;
    }
}
