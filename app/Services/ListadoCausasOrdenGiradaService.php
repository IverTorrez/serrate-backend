...
class ListadoCausasOrdenGiradaService
{
    //Servicio de listado de causas que tienen ordenes giradas
    public function listarCausasConOrdenesGiradasUsuario(Request $request)
    {
    ...
    public function listarCausasConOrdenesGiradasAbogado(Request $request)
    {
        $usuarioId = Auth::user()->id;
        $query = Causa::where('es_eliminado', 0)
    ...
    public function listarCausasConOrdenesGiradasProcurador(Request $request)
    {
        $query = Causa::where('es_eliminado', 0)
    ...
    public function listarCausasConOrdenesGiradasGeneral(Request $request)
    {
        $query = Causa::where('es_eliminado', 0)
            ->whereHas('ordenes', function ($query) {
                $query->where('etapa_orden', EtapaOrden::GIRADA)
    ...
    public function devuelveListadoCausasOrdenGirada(Request $request)
    {
        $tipoUsuario = Auth::user()->tipo;
        if ($tipoUsuario === TipoUsuario::ABOGADO_INDEPENDIENTE || $tipoUsuario === TipoUsuario::ABOGADO_LIDER); {
            $causas = $this->listarCausasConOrdenesGiradasUsuario($request);
        }
        if ($tipoUsuario === TipoUsuario::ABOGADO_DEPENDIENTE) {
            $causas = $this->listarCausasConOrdenesGiradasAbogado($request);
        }
        if ($tipoUsuario === TipoUsuario::PROCURADOR) {
            $causas = $this->listarCausasConOrdenesGiradasProcurador($request);
        }
        if ($tipoUsuario === TipoUsuario::ADMINISTRADOR || $tipoUsuario === TipoUsuario::CONTADOR || $tipoUsuario === TipoUsuario::OBSERVADOR || $tipoUsuario === TipoUsuario::PROCURADOR_MAESTRO) {
            $causas = $this->listarCausasConOrdenesGiradasGeneral($request);
        }
        return $causas;
    }
}
