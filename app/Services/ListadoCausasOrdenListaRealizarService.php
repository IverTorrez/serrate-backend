...
class ListadoCausasOrdenListaRealizarService
{
    //Servicio de listado de causas que tienen ordenes listas realizar
    public function listarCausasConOrdenesListaRealizarUsuario(Request $request)
    {
    ...
    public function listarCausasConOrdenesListaRealizarAbogado(Request $request)
    {
        $usuarioId = Auth::user()->id;
        $query = Causa::where('es_eliminado', 0)
    ...
    public function listarCausasConOrdenesListaRealizarProcurador(Request $request)
    {
        $query = Causa::where('es_eliminado', 0)
    ...
    public function listarCausasConOrdenesListaRealizarGeneral(Request $request)
    {
        $query = Causa::where('es_eliminado', 0)
            ->whereHas('ordenes', function ($query) {
                $query->whereNotNull('fecha_recepcion')
    ...
    public function devuelveListadoCausasOrdenListaRealizar(Request $request)
    {
        ...
        if ($tipoUsuario === TipoUsuario::ABOGADO_DEPENDIENTE) {
            $causas = $this->listarCausasConOrdenesListaRealizarAbogado($request);
        }
        if ($tipoUsuario === TipoUsuario::PROCURADOR) {
            $causas = $this->listarCausasConOrdenesListaRealizarProcurador($request);
        }
        if ($tipoUsuario === TipoUsuario::ADMINISTRADOR || $tipoUsuario === TipoUsuario::CONTADOR || $tipoUsuario === TipoUsuario::OBSERVADOR || $tipoUsuario === TipoUsuario::PROCURADOR_MAESTRO) {
            $causas = $this->listarCausasConOrdenesListaRealizarGeneral($request);
        }
        return $causas;
    }
}
